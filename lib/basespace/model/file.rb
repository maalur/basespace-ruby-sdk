  ...
  module BaseSpace
    ...
    class File < Model
      ...
      def download_file(api, local_dir, range = [])
        if range.empty?
          api.file_download(get_attr('Id'),local_dir, get_attr('Name'))
        else
          api.file_download(get_attr('Id'),local_dir, get_attr('Name'), range)
        end
      end
      ...
      def get_interval_coverage(api, chrom, start_pos, end_pos)
        ...
        api.get_interval_coverage(get_attr('Id'), chrom, start_pos, end_pos)
      end
      ...
      def filter_variant(api, chrom, start_pos, end_pos, qp = nil)
        ...
        api.filter_variant_set(get_attr('Id'), chrom, start_pos, end_pos, 'txt')
      end
      ...
      def get_coverage_meta(api, chrom)
        ...
        is_valid_file_option('bam')
        set_attr('Id', get_attr('HrefCoverage').split('/').last)
        api.get_coverage_meta_info(get_attr('Id'), chrom)
      end
      ...
      def get_variant_meta(api)
        ...
        is_valid_file_option('vcf')
        set_attr('Id', get_attr('HrefVariants').split('/').last)
        api.get_variant_metadata(get_attr('Id'), 'txt')
      end
    end
  end
end
