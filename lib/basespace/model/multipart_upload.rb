...
module Bio
  module BaseSpace
    ...
    class MultipartUpload
      ...
      def hasFinished
        self.Status == 'Initialized' ? 0 : !(self.getRunningThreadCount > 0)
      end
      ...
      def getRunningThreadCount
        sum(self.consumers.map(&:is_alive))
      end

      def getTransRate
        # tasks completed                        size of file-parts 
        # NOTE Python sublist [:6] already ported to Ruby [0..5]
        str((self.totalTask - self.tasks.qsize)*self.partSize/self.getRunningTime)[0..5] + ' mb/s'
      end
        
      def getRunningTime
        self.StartTime == -1 ? 0 : time.time - self.StartTime
      end
      
      # Returns the total data amoun transfered in Gb
      def getTotalTransfered
        float((self.totalTask - self.tasks.qsize) * self.partSize) / 1000.0
      end
        
      def getProgressRatio
        currentQ = float(self.tasks.qsize - len(self.consumers))
        # NOTE Python sublist [:6] already ported to Ruby [0..5]
        str(float(self.totalTask - currentQ) / self.totalTask)[0..5]
      end
